...
ws.on("connection", (sckt)=> {
    ...
    if(!sessionTb.IsTokenThere(token)){
        ...
        sckt.emit("add-token", {"token": token, "username": username});
    }
    ...
    console.log(`token : ${token}, socketId: ${sckt.id}, username: ${username}`);
    ...
    sckt.on("msg/broadcast", (username, msg) => {
        ...
        for (let onlineScktId in list){
            ...
            console.log(`msg/broadcast : [${sckt.id}, ${username}] --> ${onlineScktId}`)
        }
        console.log(`msg/broadcast done socketId: ${sckt.id}, username: ${username}`);
    })
